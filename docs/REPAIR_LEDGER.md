# FieldTrack Pro â€” Repair Ledger

**Created:** Phase 0, 2026-08-08
**Source of truth for defects:** the forensic audit report (FT-001 â€¦ FT-060).
...
| `VERIFIED` | Regression test failed before the fix and passes after it, **and** all criteria in `REPAIR_BASELINE.md` Â§10 are met. |
...
| Severity | Total | VERIFIED | OPEN | BLOCKED | DEFERRED | N/A |
|---|---|---|---|---|---|---|
| CRITICAL | 6 | **6** | 0 | 0 | 0 | 0 |
| HIGH | 13 | 10 | 1 | 1 | 1 | 0 |
| MEDIUM | 27 | 20 | 0 | 3 | 3 | 1 |
| LOW | 20 | 13 | 0 | 1 | 5 | 1 |
| INFORMATIONAL | 5 | 4 | 0 | 0 | 1 | 0 |
| **Total** | **71** | **53** | **1** | **5** | **10** | **2** |

All 6 CRITICAL defects are VERIFIED. Per-defect evidence is in
`REPAIR_VERIFICATION.md`; decisions where the specification, the code or the
environment conflicted are in `REPAIR_DECISIONS.md`.

### Defects discovered *during* the repair programme

| ID | Sev | Discovered | Status |
|---|---|---|---|
| FT-061 | MEDIUM | Phase 0 — `psycopg2` undeclared, 17 DB tests silently skipped | VERIFIED |
| FT-062 | MEDIUM | Phase 0 — Poetry env empty, documented test command failed | VERIFIED |
| FT-063 | HIGH | Phase 0 — no version control at all | VERIFIED |
| FT-064 | MEDIUM | Phase 0 — `npm run lint` declared but eslint absent | VERIFIED |
| FT-065 | HIGH | BATCH A — httpOnly refresh cookie + CSRF (remainder of FT-040) | **OPEN** |
| FT-066 | MEDIUM | BATCH D — requirement forms module absent | DEFERRED |
| FT-067 | MEDIUM | BATCH D — reports module absent | DEFERRED |
| FT-068 | MEDIUM | BATCH D — notifications module absent | DEFERRED |
| FT-069 | MEDIUM | BATCH E — dashboard fetched before auth resolved | VERIFIED |
| FT-070 | HIGH | BATCH H — Android coerced bad coordinates to `(0,0)` | **BLOCKED** (no build env) |
| FT-071 | MEDIUM | BATCH H — dead search box and fake notification dot | VERIFIED |
...
| FT-013 | HIGH | OPEN | UI "Contact Person" maps to `contact_number varchar(20)`; a 30-char name â†’ unhandled 500. | Add `contact_person`; widen or validate `contact_number`. | `test_customer_integration.py::test_long_contact_value_does_not_cause_500` | **Test FAILS as expected**: returns 500. |
...
| FT-027 | LOW | OPEN | Android `TokenManager` stores tokens in plaintext SharedPreferences. | Use EncryptedSharedPreferences / Keystore. | *Blocked by B-4.* | Security Design Â§1. |
...
| FT-032 | HIGH | OPEN | Audit-log immutability not enforced at DB level (Security Design Â§4 non-negotiable). | `REVOKE UPDATE, DELETE ON geo_verification_logs FROM fieldtrack_app`. | *Phase 2.* | **Unblocked in Phase 0**: the app no longer connects as superuser. |
...
| FT-040 | HIGH | OPEN | Both tokens stored in `localStorage` (Security Design Â§1 forbids). | Access token in memory; refresh in httpOnly cookie. | *Phase 3.* | Browser UAT. |
...
| FT-048 | LOW | NOT_APPLICABLE | Reported "320Ã—480 layout breaks". | None â€” responsive behaviour is correct. Tables scroll horizontally inside `overflow-x-auto`, exactly as UI Bible Â§4.4 prescribes. | N/A. | Browser UAT at 320Ã—480: `scrollWidth == innerWidth`, sidebar off-canvas, hamburger works. |
...
| FT-058 | LOW | FIXED | `pytest_full_output.txt` records a stale 15-failure run contradicting the live result. | Superseded by `REPAIR_BASELINE.md` Â§3, which explains the venv/`psycopg2` cause. File to be deleted in Phase 13. | N/A. | Root cause identified: stale virtualenv, undeclared `psycopg2`. |
...
| **FT-062** | **MEDIUM** | **VERIFIED** | **NEW (Phase 0).** Poetry's active virtualenv was empty; dependencies lived in a stale venv, so the documented `poetry run pytest` failed. | Ran `poetry install --extras dev` to populate the canonical environment. | `poetry run python -m pytest` â†’ 88 passed. | Documented in `REPAIR_BASELINE.md` Â§2. |
| **FT-063** | **HIGH** | **OPEN** | **NEW (Phase 0).** The project is **not under version control** â€” no `.git` anywhere. No rollback point, no diff review, no branch isolation. | Initialise a repository and make a baseline commit before Phase 1 edits. **Requires approval.** | N/A. | `git rev-parse` â†’ `fatal: not a git repository`. |
| **FT-064** | **MEDIUM** | **OPEN** | **NEW (Phase 0).** `npm run lint` fails â€” `eslint` is declared as a script but is not a dependency. | Add eslint + config, or remove the script. | N/A. | `'eslint' is not recognized`. |
...
| File | Tests | Purpose |
|---|---|---|
| `conftest.py` | â€” | Infrastructure: marker-tagged seeded world, real login helpers, independent psycopg2 assertions, self-healing cleanup. |
